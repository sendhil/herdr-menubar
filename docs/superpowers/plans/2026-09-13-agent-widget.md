...
### Saved configuration resolution fix

The missing My Shiny Agent was present in a fresh six-row App Group snapshot, including its blocked status and recorded human-message timestamp. The extension failed before provider execution: `Failed to create LinkAction`, `Unable to get LNAction from intent`, `No AppIntent in timeline(for:with:)`, then CHSErrorDomain 1101. The saved AppIntentDescriptor used host BundleIdentifier dev.herdr.menubar, but the host had no AppIntent metadata. Restarting the extension and removing obsolete development registrations did not resolve the failure.

Moved AgentWindow's AppEnum conformance and AgentWidgetConfiguration into a shared file compiled by both host and extension. Incremented both build versions to 3. Verified generated metadata in both signed installed bundles contains AgentWidgetConfiguration (host metadata was absent before). The unchanged existing widget successfully loaded at 19:38:51 and again at 19:38:53; no re-add was needed. This validates recovery of saved configuration resolution, not an unlimited guarantee against future OS refresh throttling.
